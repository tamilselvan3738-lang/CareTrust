...
# Same replacements we planned for index.html, but now apply to all .html files
...
def replace_in_file(filepath):
    ...
    # Colors
    content = content.replace("'primary': '#EF4444'", "'primary': '#000080'")
    content = content.replace("'secondary': '#F97316'", "'secondary': '#000000'")
    content = content.replace("'gradient-primary': 'linear-gradient(to right, #EF4444, #F97316)'", "'gradient-primary': 'linear-gradient(to right, #000080, #000000)'")

    # Titles and icons
    content = content.replace("IronPulse | Transform Your Body", "CareTrust | Home Nursing & Patient Care")
    content = content.replace("IronPulse", "CareTrust")
    content = content.replace("Iron<span class=\"text-primary\">Pulse</span>", "Care<span class=\"text-primary\">Trust</span>")
    content = content.replace("fa-dumbbell", "fa-heartbeat")
    content = content.replace("© 2026 IronPulse Fitness. All rights reserved.", "© 2026 CareTrust Nursing. All rights reserved.")

    # Navbar Desktop Links
    desktop_links_old = '''<a href="about.html" class="text-xs uppercase tracking-[0.1em] font-medium text-dark-bg dark:text-white hover:text-primary dark:hover:text-primary transition-colors px-2 py-2 rounded-md hover:bg-gray-100 dark:hover:bg-primary/10">About</a>
    ...
    desktop_links_new = '''<a href="about.html" class="text-xs uppercase tracking-[0.1em] font-medium text-dark-bg dark:text-white hover:text-primary dark:hover:text-primary transition-colors px-2 py-2 rounded-md hover:bg-gray-100 dark:hover:bg-primary/10">About Us</a>
    ...
    # Navbar Mobile Links
    mobile_links_old = '''<a href="about.html" class="block py-4 border-b border-gray-200 dark:border-white/10 text-dark-bg dark:text-white hover:text-primary text-xs uppercase tracking-widest font-medium">About</a>
    ...
    mobile_links_new = '''<a href="about.html" class="block py-4 border-b border-gray-200 dark:border-white/10 text-dark-bg dark:text-white hover:text-primary text-xs uppercase tracking-widest font-medium">About Us</a>
    ...
    # Remove Dashboard Menu completely
    dash_regex = re.compile(r'<!-- Dashboard Dropdown -->.*?</div>\s*</div>', re.DOTALL)
    content = dash_regex.sub('', content)

    mobile_dash_regex = re.compile(r'<div class="border-b border-gray-200 dark:border-white/10 flex flex-col">\s*<div class="w-full flex items-center justify-between py-4">\s*<span class="text-xs uppercase tracking-widest font-medium text-dark-bg dark:text-white">Dashboard</span>.*?</div>\s*</div>', re.DOTALL)
    content = mobile_dash_regex.sub('', content)

    # New 6 Sections
    new_main = '''<main class="flex-grow w-full max-w-[100vw] overflow-hidden">
    <!-- 1. HERO SECTION -->

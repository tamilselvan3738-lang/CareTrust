...
def get_header(active=''):
    ...
    home1_item_class = "bg-blue-600 text-white font-extrabold shadow-sm" if is_home1 else "text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 hover:text-blue-600 dark:hover:text-sky-400 font-medium"
    home2_item_class = "bg-blue-600 text-white font-extrabold shadow-sm" if is_home2 else "text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 hover:text-blue-600 dark:hover:text-sky-400 font-medium"
      ...
      <div class="flex items-center justify-between h-20">
        ...
        <nav class="hidden lg:flex items-center gap-1 xl:gap-2">
          ...
          <div class="relative group">
            ...
            <div class="absolute top-full start-0 w-60 py-2 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
        ...
        <div class="hidden sm:flex items-center gap-2.5">
          <!-- RTL Button (Identical w-9 h-9 size to Theme Toggle) -->
          ...
          <!-- Theme Toggle Button (Identical w-9 h-9 size to RTL) -->
          ...
          <!-- Login Option Styled as Secondary CTA -->
          ...
          <!-- Register / Consultation Styled as Primary CTA -->
    ...
    <div id="mobile-drawer" class="hidden lg:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 pt-3 pb-6 space-y-2 shadow-2xl">
      ...
      <div class="pt-4 flex flex-col gap-2.5">
        <div class="flex gap-2">
          <button type="button" class="rtl-toggle-btn flex-1 py-2.5 text-xs font-bold rounded-xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">Toggle RTL</button>
        ...
        <a href="register.html" class="block w-full py-3 text-center text-xs font-bold uppercase tracking-wider text-white bg-gradient-to-r from-blue-600 to-sky-500 rounded-xl shadow-md">Register Account</a>
      </div>
    </div>
  </header>
"""
  ...
  <script>
    ...
    rtlButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        htmlElement.setAttribute('dir', curDir === 'ltr' ? 'rtl' : 'ltr');
...
def assemble_page(filename, title, desc, active, main_content):
    content = get_head(title, desc) + get_header(active) + f"<main class='flex-grow'>{main_content}</main>" + get_footer()
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Generated {filename}")

if __name__ == '__main__':
    print("build_pages module updated.")
